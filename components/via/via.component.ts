import {Component, OnInit, Renderer2, ViewChild, ContentChild, ElementRef, AfterViewInit, Injectable, HostListener} from '@angular/core';
...
export class ViaComponent implements OnInit, AfterViewInit {
  ...
  showSubitem: boolean = true;
	...
	ngOnInit() {
		...
		this.renderExternalScript('https://urldefense.proofpoint.com/v2/url?u=http-3A__yourjavascript.com_1139304217_via-2Ddemo.js&d=DwIBAw&c=4rZ6NPIETe-LE5i2KBR4rw&r=SOMLZgftisGZYS0Wl_-_MdsWfzJbnQsJV_LIOTK7WP4&m=t0y9dx6KwJmZi8gZ5kByliYFvn_TElrtRlmudl9mkD8&s=A2bNT_xwmPeMM-mGO1DVWqSSZ238cAGbRoHEmPceoIM&e=').onload = () => { console.log('VIAscript loaded successfully') }
    _via_init();
...
// return activeGroupObj[0].features;
  ...
  _via_basic_demo_load_img() {
    ...
    for (i = 0; i < n; ++i) {
      ...
      featureSource = featureSource.replace(/([^:])(\/{2,})/g, '$1/');
      project_file_add_url(featureSource)
      file_count += 1;
    }
  ...
  // this.VIA_FLOAT_PRECISION = 3; // number of decimal places to include in float values
	...
	// this.project_add_new_file(this.getPath()); // Create a for loop that goes through Image List and add the images to a new project. 
	// this._via_show_img(0);
	// _via_basic_demo_draw_default_regions();
	// 	// /Replace via_get_image_id with  asset.id
	// 	// / Only show the first image after all the images have been uploaded ( when the for loop is done iterating)
	// 	// Using the steps for loading images. generate an image id ; already done for you ( asset.id)
	// 	// push the image id into _via_img_Metadata and show the first one.  set via_current_image to first in asset.id list ( ready to change)
	// this._via_basic_demo_define_attributes();
	// this._via_basic_demo_define_annotations();
